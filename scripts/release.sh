...
release-notes --org mattermost --repo mattermost-cloud-database-factory --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch master
...
echo "The image for this release is \`docker.io/mattermost/mattermost-cloud-database-factory:${GITHUB_REF_NAME}\`" >> ./build/_output/docs/relnote_parsed.md
echo "\n" >> ./build/_output/docs/relnote_parsed.md
...
hub release create -d -F ./build/_output/docs/relnote.md ${GITHUB_REF_NAME}
rm -rf ./build/_output/
